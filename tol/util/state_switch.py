...
class StateSwitch:
    def __init__(self, state_names, current_time):
        ...
        self.timers = Timers(['switch'], current_time)
    ...
    def update(self, current_time):
        ...
        if self.timers.is_it_time(
                'switch',
                current_state.get_duration(),
                current_time):
            self.timers.reset('switch', current_time)
            self.current_state_name = current_state.get_next_state_name()
        return self.current_state_name

    def switch_to_state(self, state_name):
