...
check_output(
    'protoc -I {GAZEBO_INCLUDE_DIR} -I {GAZEBO_PROTO_DIR} -I {PKG_DIR} --python_out={PKG_DIR}/ {PKG_DIR}/{PROTO_DIR}/*.proto'
    .format(
...
patt = "^import\s+((?!{PROTO_DIR}\.).*_pb2)".format(PROTO_DIR = proto_dir)
gazebo_package_finder = re.compile(patt, re.MULTILINE)
